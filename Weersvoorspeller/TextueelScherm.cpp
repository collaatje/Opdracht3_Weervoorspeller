...
TextueelScherm::TextueelScherm( WeerData* weerData )
{
	...
	this->weerDataLabel = new Label(5, 50, 230, 200, achtergrond, "Weersverwachting:", 0, font);
	weerDataLabel->setSkin( this->skin );


	//info label
	this->weerInfo = new Label(5, 80, 230, 30, achtergrond, "Verw.: vnd, morgen, overmorgen", 0 , font);
...
void TextueelScherm::toonWeerData()
{
	...
	sprintf( data, "Minimum: %i	/ %i / %i", weerData->minimumtemperatuur[0], weerData->minimumtemperatuur[1], weerData->minimumtemperatuur[2] );
